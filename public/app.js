...
function shotsPage() {
  ...
  const card = shot => {
    ...
    const motionReady = shot.shotType === 'motion' && resolved.camera?.enabled !== false && startPreset && endPreset && Number.isInteger(startPreset.presetNumber) && Number.isInteger(endPreset.presetNumber) && cameraAdapterSupported(resolved.camera);
  ...
  const motionPresetSummary = (label, preset) => `<div class="motion-preset-status"><span class="eyebrow">${label}</span><strong>${escapeHtml(preset?.name || `Missing ${label}`)}</strong><small>${Number.isInteger(preset?.presetNumber) ? `Hardware Preset ${preset.presetNumber}` : 'Needs Setup · Hardware preset number missing'}</small><small>${escapeHtml(resolved?.camera?.name || 'Camera missing')}</small></div>`;
  const motionErrors = selectedType === 'motion' ? [
    ...
    resolved?.preset && !Number.isInteger(resolved.preset.presetNumber) ? 'Start preset needs a hardware preset number' : null,
    selectedEndPreset && !Number.isInteger(selectedEndPreset.presetNumber) ? 'End preset needs a hardware preset number' : null,
  ...
  document.getElementById('motion-manage-presets')?.addEventListener('click', () => {
...
function settingsPage() {
  ...
  } else if (settingsSection === 'video') {
    const defaultTransition = state.settings?.videoSwitching?.defaultTransition === 'cut' ? 'cut' : 'mix';
    body = `<div class="settings-heading"><div><span class="eyebrow">VIDEO SWITCHING</span><h1>Video Switching</h1><p>Choose the transition Trinity uses for normal Take Live actions. Physical switcher buttons are unaffected.</p></div></div>
      <section class="panel settings-form"><label>Default Transition<select id="video-default-transition"><option value="mix" ${defaultTransition === 'mix' ? 'selected' : ''}>Mix</option><option value="cut" ${defaultTransition === 'cut' ? 'selected' : ''}>Cut</option></select><small>Mix uses the transition rate currently configured on the active switcher.</small></label></section>`;
  ...
  document.getElementById('video-default-transition')?.addEventListener('change', async event => {
    state = await window.trinity.updateVideoSwitchingSettings({ defaultTransition: event.target.value });
    showNotification(`Default video transition set to ${event.target.value === 'cut' ? 'Cut' : 'Mix'}`, { type: 'success' });
    render();
  });

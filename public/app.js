...
function livePage(){
  ...
  shell(`<div class="live-main">
    ...
    <section class="card stack">
      <div>
        ...
        <div class="hero">${esc(cue?.name||'No Cue')}</div>
        <div class="muted">${esc(cue?.notes||'')}</div>
      </div>
      ...
      <div class="live-camera-console">
        ...
        <div class="live-camera-grid">${state.cameras.map(camera=>{
          ...
          const selectedPreset=isProgram
            ? (state.live.programPreset||state.presetNames[0])
            : isPreview
              ? (state.live.previewPreset||state.presetNames[0])
              : (state.presets[camera.id]?.[0]?.name||state.presetNames[0]);

          return `<div class="live-camera ${isProgram?'program':''} ${isPreview?'preview':''}">
            <div class="camera-role ${isProgram?'on-program':''}">
              ${isProgram?'● PROGRAM':isPreview?'● PREVIEW':'CAMERA'}
            </div>
      ...
      <div class="grid three">
        <div>
          ...
          <div class="monitor program">${esc(byId(state.cameras,state.live.programCamera)?.name||'None')}
            <div class="muted">${esc(state.live.programPreset||'')}</div>
          </div>
        </div>
        <div>
          <div class="kicker">Preview</div>
          <div class="monitor preview">${esc(byId(state.cameras,state.live.previewCamera)?.name||'None')}
            <div class="muted">${esc(state.live.previewPreset||'')}</div>
          </div>
        </div>
        <div>
          <div class="kicker">Next</div>
          <div class="monitor">${esc(next?.name||'End')}</div>
        </div>
      </div>

      <div>
        <div class="kicker">Production State</div>
        <div class="actions">
      ...
      <div class="scene-grid">${state.lightingScenes.map(s=>`
        <button data-light="${s.id}" class="${state.live.lightingOverrideId===s.id?'warn':''}">${esc(s.name)}</button>
      `).join('')}</div>

      <div class="actions">
        <button class="go" id="next">TAKE NEXT</button>
  ...
  document.querySelectorAll('[data-live-go]').forEach(el=>{
    el.onclick=async()=>{state=await window.trinity.goCue(Number(el.dataset.liveGo));render()};
  });

  document.querySelectorAll('[data-light]').forEach(button=>{
    button.onclick=async()=>{state=await window.trinity.lightingOverride(button.dataset.light);render()};
  });

  document.querySelectorAll('[data-program-camera]').forEach(button=>{
  ...
  document.querySelectorAll('[data-camera-preset]').forEach(select=>{
    select.onchange=async()=>{
      const cameraId=select.dataset.cameraPreset;

      if(cameraId===state.live.programCamera){
        state.live.programPreset=select.value;
      }else{
        state.live.previewCamera=cameraId;
        state.live.previewPreset=select.value;
      }
  ...
  if(returnButton){
    returnButton.onclick=async()=>{state=await window.trinity.returnToCueLighting();render()};
  }

  document.getElementById('blackout').onclick=()=>alert('Blackout is simulated in Alpha 3.');
}

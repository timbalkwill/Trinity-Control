(() => {
  ...
  function render() {
    ...
    root.innerHTML = `<div class="operator compact-operator">
      ...
      <section class="panel"><div class="eyebrow">CAMERA READINESS</div><div class="camera-safe-summaries">${(state.managedCameras || []).map(camera => `<div><strong>${escapeHtml(camera.displayName)}</strong><span>${escapeHtml(camera.logicalRole || "camera")} · ${escapeHtml(camera.readiness || "Unknown")}</span><small>${camera.programState ? "PROGRAM" : camera.previewState ? "PREVIEW" : "Standby"} · Preset ${escapeHtml(camera.currentPresetName || "Unknown")} · Tracking ${escapeHtml(camera.trackingState || "Unknown")} · Motion ${escapeHtml(camera.motionState || "Unknown")}${camera.warning ? ` · ${escapeHtml(camera.warning)}` : ""}</small></div>`).join("") || '<span class="muted">No camera summaries available</span>'}</div></section>

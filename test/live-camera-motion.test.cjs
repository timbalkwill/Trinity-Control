...
test("renderer uses authoritative shot type and camera ID with independent scrolling and truthful feedback", () => {
  ...
  assert.match(card, /data-run-motion-camera=.*camera\.id/);
  assert.match(card, /data-run-motion-shot=.*shot\.id/);
  assert.match(card, /Last Commanded Motion/);
  assert.match(renderer, /window\.trinity\.runCameraMotion/);
  assert.match(renderer, /Executing…/);
